...
public abstract class AbstractPDFDocument implements PDFDocument {
	...
	private static final Logger logger = LoggerFactory.getLogger(AbstractPDFDocument.class);

	@Override
    public int getMostVisiblePage(final Rectangle2D viewBounds)
	{
		final Collection<Integer> visiblePages = getVisiblePages(viewBounds);

		if (logger.isDebugEnabled()) {
			for (final Iterator<Integer> pageIterator = visiblePages.iterator(); pageIterator.hasNext();) {
				final int pageNumber = pageIterator.next();
				logger.debug(String.valueOf(pageNumber));
			}
		}
...
/*		if (visiblePages.isEmpty())
		...
		for (final Iterator<Integer> pageIterator = visiblePages.iterator(); pageIterator.hasNext();) {
			...
			final Rectangle2D pageBounds = getPageBounds(pageNumber);
			int visiblePageArea;
        	...
        	if (viewBounds.contains(pageBounds)) {
        		...
        		final double x = Math.max(pageBounds.getX(), viewBounds.getX());
        		final double y = Math.max(pageBounds.getY(), viewBounds.getY());
        		final double width = Math.min(pageBounds.getMaxX() - x, viewBounds.getMaxX() - x);
        		final double height = Math.min(pageBounds.getMaxY() - y, viewBounds.getMaxY() - y);

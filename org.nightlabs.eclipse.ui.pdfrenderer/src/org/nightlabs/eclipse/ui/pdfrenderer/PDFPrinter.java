...
/**
 * Handles the printing of PDF files. Throw-away instances of this class are
 * used to print PDF documents.
 * 
 * @version $Revision: 349 $ - $Date: 2008-10-13 15:32:46 +0200 (Mon, 13 Oct
 *          2008) $
...
public class PDFPrinter {
	 ...
	 *            the file to print.
	 */
	public void printPDF(final File file, final PrinterJob printerJob, final boolean usePDFPageDimensions)
	...
	public void printPDF(final PDFFile pdfFile, final PrinterJob printerJob, final boolean usePDFPageDimensions)
		...
		// Creates a new PageFormat instance and sets it to a default size and
		// orientation.
	...
	private class PDFPageable implements Pageable {
		...
		/**
		 * Gets the {@link PageFormat} for the current page of the given
		 * {@link PDFFile}.
		 * 
		 * @param pageIndex
		 *            the number of the current page (zero-based) to get the
		 *            {@link PageFormat} for.
		...
		public PageFormat getPageFormat(final int pageIndex) throws IndexOutOfBoundsException {
			...
			final PDFPage pdfPage = pdfFile.getPage(pageIndex + 1); // pageIndex
																	// is
																	// zero-based!

			final Paper paper = new Paper();
			// Sets the width and height of the Paper object to the measures of
			// the default page (see above).
			...
			// Sets the imageable area (the area in which printing occurs) of
			// the Paper object according to the
			// measures of the default page. The imageable area of the paper
			// begins directly at the top-left point.
			paper.setImageableArea(0, 0, usePDFPageDimensions ? pdfPage.getBBox().getWidth() : defaultPage.getWidth(),
					usePDFPageDimensions ? pdfPage.getBBox().getHeight() : defaultPage.getHeight());

			final PageFormat pageFormat = new PageFormat();
			// Sets the orientation of the PageFormat object to the one of the
			// currently considered PDF page.
		...
		/**
		 * Gets a {@link Printable} for the current page of the given
		 * {@link PDFFile}.
		 * 
		 * @param pageIndex
		 *            the number of the current page (zero-based) to get the
	...
	private class PDFPrintable implements Printable {
		...
		/**
		 * Prints the page at the specified index (zero-based) into the
		 * specified {@link Graphics} context in the specified format.
		 * 
		 * @param graphics
		 *            the context into which the page is drawn.
		 * @param pageFormat
		 *            the {@link PageFormat} (size and orientation) of the
		 *            current page to be drawn.
		 * @param pageIndex
		 *            the number of the current page to be drawn (zero-based).
		 * @return PAGE_EXISTS if the page is rendered successfully or
		 *         NO_SUCH_PAGE if <code>pageIndex</code> specifies a
		 *         non-existent page.
		 */
		@Override
		public int print(final Graphics graphics, final PageFormat pageFormat, final int pageIndex)
			...
			/**
			 * The factors widthScaleFactor and heightScaleFactor are used for
			 * scaling the given {@link PDFPage} to fit the paper dimensions
			 * described by the default page. Only the smallest factor
			 * minScaleFactor of both will be taken into consideration for
			 * scaling.
			 */
			...
			// Gets the dimensions of the current PageFormat (i.e. the
			// dimensions of the default page).
			final Point2D.Double defaultPageDimensions = new Point2D.Double(pageFormat.getWidth(),
			...
			// Gets the dimensions of the page bounds of the current PDF page
			// before applying a screenResolutionFactor.
			final double pageWidth = pdfPage.getBBox().getWidth();
			final double pageHeight = pdfPage.getBBox().getHeight();

			// TODO: Alex: Frederik, what does the screen-resolution have to do
			// with printing? Is the following, commented code really necessary?
			...
			final Point2D.Double screenResolutionFactor = new Point2D.Double((double) screenDPI[0].x / 72,
					(double) screenDPI[0].y / 72);

			// Sets the dimensions of the page bounds of the current PDF page
			// after applying the computed screenResolutionFactor.
			final Point2D.Double pageBoundsImage = new Point2D.Double((pageWidth) * screenResolutionFactor.getX(),
			...
			// Tests if scaling has to be done to fit the current PDF page to
			// the dimensions of default page.
			...
			// Only the smallest factor of both factors will be taken into
			// consideration and used for scaling.
			...
			// The image bounds into which to fit the page are described by a
			// rectangle using the (possibly scaled)
			// page bounds as bounds.
			// A clipping will not be used as the whole page should be drawn.
			final PDFRenderer renderer = new PDFRenderer(pdfPage, graphics2D, new Rectangle(0, 0,
					(int) pageBoundsImage.x, (int) pageBoundsImage.y), null, // do
																				// not
																				// use
																				// any
																				// clipping
	...
	// // printer.printDocument(new
	// File("/home/frederik/pdfs/recommender-systems.pdf"));
	// // printer.printDocument(new
	// File("/home/frederik/pdfs/numerics0304.pdf"));
	// // printer.printDocument(new
	// File("/home/frederik/pdfs/landscape-format.pdf"));
	//
	// }
	 ...
	 * 
	 * @param pdfPage
	 *            the PDF page to get the orientation from.

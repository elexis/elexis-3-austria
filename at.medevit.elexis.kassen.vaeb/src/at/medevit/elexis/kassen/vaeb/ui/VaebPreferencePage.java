...
public class VaebPreferencePage extends FieldEditorPreferencePage implements
	...
	public void init(IWorkbench workbench){
		...
		setDescription("Einstellungen zur Versicherungsanstalt für Eisenbahnen und Bergbau");
	}
	...
	protected Control createContents(final Composite parent){
		...
		// create a composite for selecting the contact of the vgkk
		...
		contactLbl.setText("Kontakt der VAEB:");
		contactTxt = new Text(contactParent, SWT.BORDER);
		contactTxt.setEditable(false);
		String vaebContactId =
			getPreferenceStore().getString(
				VaebPreferenceInitializer.VAEBPREF + CorePreferenceConstants.KASSE_CONTACT);
		if (vaebContactId.length() > 1) {
			Kontakt vaebContact = Kontakt.load(vaebContactId);
			contactTxt.setText(vaebContact.getLabel());
		}
			...
			public void widgetSelected(SelectionEvent e){
				KontaktSelektor ksl =
					new KontaktSelektor(getShell(), Kontakt.class, "Kontakt auswählen",
						"Kontakt der VAEB auswählen, bzw. zuerst anlegen.", false,
		...
		openCatalog.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e){
				EditCatalogDialog dialog =
					new EditCatalogDialog(parent.getShell(), VaebLeistung.class);
				dialog.create();
				dialog.open();
			}
		});
			...
			menuManager.add(new Action() {
				...
				public void run(){
					EditPointsAreaDialog dialog =
						new EditPointsAreaDialog(parent.getShell(), VaebLeistung.class);
					dialog.create();
			...
			menuManager.add(new Action() {
				...
				public void run(){
					...
					if (selection != null & selection instanceof IStructuredSelection) {
						...
						EditPointsAreaDialog dialog =
							new EditPointsAreaDialog(parent.getShell(), sel, VaebLeistung.class);
						dialog.create();
						if (dialog.open() == Window.OK) {
							viewer.setInput(PointsAreaFactory.getInstance().getPointsAreasForClass(
								VaebLeistung.class));
	...
	protected void createFieldEditors(){
		...
		StringFieldEditor hvCodeEditor =
			new StringFieldEditor(VaebPreferenceInitializer.VAEBPREF
		...
		ComboFieldEditor insuranceCategoryEditor =
			new ComboFieldEditor(VaebPreferenceInitializer.VAEBPREF

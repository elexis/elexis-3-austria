...
import at.medevit.elexis.kassen.bva.Activator;
...
public class BvaPreferencePage extends FieldEditorPreferencePage implements
	...
	public void init(IWorkbench workbench){
		...
		setDescription("Einstellungen zur Versicherungsanstalt öffentlich Bediensteter");
	}
	...
	protected Control createContents(final Composite parent){
		...
		contactLbl.setText("Kontakt der BVA:");
		contactTxt = new Text(contactParent, SWT.BORDER);
		contactTxt.setEditable(false);
		String bvaContactId =
			getPreferenceStore().getString(
				BvaPreferenceInitializer.BVAPREF + CorePreferenceConstants.KASSE_CONTACT);
		if (bvaContactId.length() > 1) {
			Kontakt bvaContact = Kontakt.load(bvaContactId);
			...
			public void widgetSelected(SelectionEvent e){
				KontaktSelektor ksl =
					new KontaktSelektor(getShell(), Kontakt.class, "Kontakt auswählen",
						"Kontakt der BVA auswählen, bzw. zuerst anlegen.", false,
		...
		openCatalog.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e){
				EditCatalogDialog dialog =
					new EditCatalogDialog(parent.getShell(), BvaLeistung.class);
				dialog.create();
				dialog.open();
			}
		});
		...
		try {
			...
			viewer.setInput(PointsAreaFactory.getInstance().getCurrentPointsAreasForClass(
				BvaLeistung.class));
			...
			menuManager.add(new Action() {
				...
				public void run(){
					EditPointsAreaDialog dialog =
						new EditPointsAreaDialog(parent.getShell(), BvaLeistung.class);
					dialog.create();
			...
			menuManager.add(new Action() {
				...
				public void run(){
					...
					if (selection != null & selection instanceof IStructuredSelection) {
						...
						EditPointsAreaDialog dialog =
							new EditPointsAreaDialog(parent.getShell(), sel, BvaLeistung.class);
						dialog.create();
						if (dialog.open() == Window.OK) {
							viewer.setInput(PointsAreaFactory.getInstance().getPointsAreasForClass(
								BvaLeistung.class));
	...
	protected void createFieldEditors(){
		...
		StringFieldEditor hvCodeEditor =
			new StringFieldEditor(BvaPreferenceInitializer.BVAPREF
		...
		ComboFieldEditor insuranceCategoryEditor =
			new ComboFieldEditor(BvaPreferenceInitializer.BVAPREF

...
import at.medevit.elexis.kassen.vgkk.Activator;
...
public class VgkkPreferencePage extends FieldEditorPreferencePage implements
	...
	public void init(IWorkbench workbench){
		...
		setDescription("Einstellungen zur Vorarlberger Gebietskrankenkasse");
	}
	...
	protected Control createContents(final Composite parent){
		...
		contactLbl.setText("Kontakt der VGKK:");
		contactTxt = new Text(contactParent, SWT.BORDER);
		contactTxt.setEditable(false);
		String vgkkContactId =
			getPreferenceStore().getString(
				VgkkPreferenceInitializer.VGKKPREF + CorePreferenceConstants.KASSE_CONTACT);
		if (vgkkContactId.length() > 1) {
			Kontakt vgkkContact = Kontakt.load(vgkkContactId);
			...
			public void widgetSelected(SelectionEvent e){
				KontaktSelektor ksl =
					new KontaktSelektor(getShell(), Kontakt.class, "Kontakt auswählen",
						"Kontakt der VGKK auswählen, bzw. zuerst anlegen.", false,
		...
		openCatalog.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e){
				EditCatalogDialog dialog =
					new EditCatalogDialog(parent.getShell(), VgkkLeistung.class);
				dialog.create();
				dialog.open();
			}
		});
			...
			menuManager.add(new Action() {
				...
				public void run(){
					EditPointsAreaDialog dialog =
						new EditPointsAreaDialog(parent.getShell(), VgkkLeistung.class);
					dialog.create();
			...
			menuManager.add(new Action() {
				...
				public void run(){
					...
					if (selection != null & selection instanceof IStructuredSelection) {
						...
						EditPointsAreaDialog dialog =
							new EditPointsAreaDialog(parent.getShell(), sel, VgkkLeistung.class);
						dialog.create();
						if (dialog.open() == Window.OK) {
							viewer.setInput(PointsAreaFactory.getInstance().getPointsAreasForClass(
								VgkkLeistung.class));
	...
	protected void createFieldEditors(){
		...
		StringFieldEditor hvCodeEditor =
			new StringFieldEditor(VgkkPreferenceInitializer.VGKKPREF
		...
		ComboFieldEditor insuranceCategoryEditor =
			new ComboFieldEditor(VgkkPreferenceInitializer.VGKKPREF
